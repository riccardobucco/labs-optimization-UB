...
ITERATIONS = 100
ALPHA = 0.05
STARTING_POINTS = np.array([[-0.6,-0.5], [0,0.3], [-1.1, 0]])
COLORS = np.array(['g', 'r', 'y'])

# Function that computes the value of the function in a point x=(x1,x2)
def myFunction(x):
    y = x[0]**2 * (4 - 2.1 * x[0]**2 + 1.0/3 * x[0]**4) + x[0]*x[1] + x[1]**2 * (-4 + 4 * x[1]**2)
    return y
...
plt.savefig('../Images/01-gradient-descent-1st-lab-function-1st-experiment.png')