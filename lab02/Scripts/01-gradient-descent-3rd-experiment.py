...
# Function that computes the value of the specified function in a point x=(x1,x2)
...
def mySecondFunction(x):
    y = 100*x[0]**2 + 100*x[1]**2
    return y

# Function that computes the gradient (gx1, gx2) of the specified function in a point x=(x1,x2)
def myFirstFunctionGradient(x):
    gradient = np.array([2*x[0], 2*x[1]])
    return gradient

# Function that computes the gradient (gx1, gx2) of the specified function in a point x=(x1,x2)
def mySecondFunctionGradient(x):
    gradient = np.array([200*x[0], 200*x[1]])
    return gradient
...
# Function that implements the normalized gradient descent method.
# It requires a starting point, alpha, a function that computes the value of a function in a specified point,
# a function that computes the gradient of a function in a specified point, the number of iterations to perform
# It returns an array of points, where each point is computed using the normalized gradient descent method.
def normalizedGradientDescent(x0, alpha, function, function_gradient, iterations):
    points = [x0]
    xk = x0
    gradient_xk = function_gradient(xk)
    for i in range(0, iterations):
        xk = xk - alpha*(gradient_xk/np.linalg.norm(gradient_xk))
        gradient_xk = function_gradient(xk)
        points.append(xk)
    return np.array(points)
...
for i in range(0, COLORS.size):
    ...
    points = gradientDescent(x0, ALPHA, myFirstFunction, myFirstFunctionGradient, ITERATIONS)
...
for i in range(0, COLORS.size):
    x0 = STARTING_POINTS[i]
    points = gradientDescent(x0, ALPHA, mySecondFunction, mySecondFunctionGradient, ITERATIONS)
    ax.plot(points[:,0], points[:,1], COLORS[i]+'o')
...
for i in range(0, COLORS.size):
    x0 = STARTING_POINTS[i]
    points = normalizedGradientDescent(x0, ALPHA, myFirstFunction, myFirstFunctionGradient, ITERATIONS)
    ax.plot(points[:,0], points[:,1], COLORS[i]+'o')
...
for i in range(0, COLORS.size):
    x0 = STARTING_POINTS[i]
    points = normalizedGradientDescent(x0, ALPHA, mySecondFunction, mySecondFunctionGradient, ITERATIONS)
    ax.plot(points[:,0], points[:,1], COLORS[i]+'o')
